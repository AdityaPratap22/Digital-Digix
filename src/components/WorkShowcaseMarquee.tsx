...
export const WorkShowcaseMarquee: React.FC<WorkShowcaseMarqueeProps> = ({ locationName, backgroundColor }) => {
  const [activeLightboxProject, setActiveLightboxProject] = useState<SocialMediaProject | null>(null);

  return (
    <section style={{ padding: '6rem 0', backgroundColor: backgroundColor || 'var(--bg-main)', overflow: 'hidden', borderBottom: '1px solid var(--border-color)', position: 'relative' }}>
      ...
      <div className="container" style={{ position: 'relative', zIndex: 1 }}>
        <div style={{ textAlign: 'center', marginBottom: '4.5rem' }}>
          <span className="section-tag" style={{ color: '#3B82F6', background: 'rgba(59, 130, 246, 0.1)' }}>CAMPAIGN SHOWCASE & CREATIVE ASSETS</span>
          <h2 style={{ fontSize: '2.8rem', fontWeight: 900, fontFamily: 'Outfit, sans-serif', color: 'var(--secondary)', marginTop: '0.5rem', marginBottom: '1rem', letterSpacing: '-0.02em' }}>
            Live Creative Campaigns & Case Studies {locationName ? `in ${locationName}` : ''}
          </h2>
          <p style={{ color: 'var(--text-muted)', fontSize: '1.05rem', maxWidth: '640px', margin: '0 auto', lineHeight: 1.6 }}>
            Explore authentic, conversion-engineered social media assets and digital campaigns produced for industry leaders {locationName ? `in ${locationName}` : ''}.
          </p>
        </div>
      </div>
