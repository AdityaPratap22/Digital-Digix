export interface CountryLocationData {
  country: string;
  capital: string;
  currency: string;
  currencyCode: string;
  region: 'North America' | 'Europe' | 'Middle East' | 'Latin America & Caribbean' | 'Pacific & Africa';
  majorCities: string[];
  regionalCities: string[];
}

/**
 * Ensures country and location names have their first letters capitalized properly.
 * Looks up canonical capitalization from ALL_COUNTRY_LOCATIONS, or capitalizes each word's first letter.
 */
export function formatLocationName(raw: string): string {
  if (!raw) return '';
  const clean = decodeURIComponent(raw)
    .replace(/^digital-marketing-in-/, '')
    .replace(/^digital-marketing-for-/, '')
    .replace(/^digital-marketing-/, '')
    .replace(/-/g, ' ')
    .trim();

  // Try to find exact match in ALL_COUNTRY_LOCATIONS
  const countryObj = ALL_COUNTRY_LOCATIONS.find(c => 
    c.country.toLowerCase() === clean.toLowerCase() ||
    c.capital.toLowerCase() === clean.toLowerCase() ||
    c.majorCities.some(city => city.toLowerCase() === clean.toLowerCase()) ||
    c.regionalCities.some(city => city.toLowerCase() === clean.toLowerCase())
  );

  let result = clean;

  if (countryObj) {
    if (countryObj.country.toLowerCase() === clean.toLowerCase()) {
      result = countryObj.country;
    } else if (countryObj.capital.toLowerCase() === clean.toLowerCase()) {
      result = countryObj.capital;
    } else {
      const matchedCity = [...countryObj.majorCities, ...countryObj.regionalCities].find(
        city => city.toLowerCase() === clean.toLowerCase()
      );
      if (matchedCity) result = matchedCity;
    }
  } else {
    const matchedFooter = TOP_FOOTER_INTERNATIONAL_LOCATIONS.find(
      loc => loc.toLowerCase() === clean.toLowerCase()
    );
    if (matchedFooter) result = matchedFooter;
  }

  // Capitalize the first letter of each word
  return result
    .split(/\s+/)
    .map(word => (word ? word.charAt(0).toUpperCase() + word.slice(1) : ''))
    .join(' ');
}

export const TOP_FOOTER_INTERNATIONAL_LOCATIONS = [
  'United States', 'United Kingdom', 'Germany', 'France', 'Switzerland', 'Netherlands',
  'Spain', 'Italy', 'Kuwait', 'Bahrain', 'Oman', 'Jordan', 'Ireland', 'Austria',
  'Belgium', 'Finland', 'Portugal', 'Ecuador', 'Panama', 'Bahamas', 'Bermuda',
  'Cayman Islands', 'Luxembourg', 'Monaco', 'Puerto Rico', 'Greece', 'Cyprus', 'Malta'
];

export const REGIONAL_FOOTER_LOCATIONS = [
  {
    title: '⭐ Tier 1 Global Metros & Markets',
    locations: ['United States', 'United Kingdom', 'Germany', 'France', 'Switzerland', 'Netherlands', 'Spain', 'Italy', 'Kuwait', 'Bahrain', 'Oman', 'Jordan', 'Ireland', 'Austria', 'Belgium', 'Finland', 'Portugal']
  },
  {
    title: '🏰 Western & Northern Europe',
    locations: ['United Kingdom', 'Germany', 'France', 'Italy', 'Spain', 'Netherlands', 'Switzerland', 'Ireland', 'Austria', 'Belgium', 'Portugal', 'Finland', 'Greece', 'Luxembourg', 'Malta', 'Cyprus', 'Monaco', 'Liechtenstein', 'Andorra', 'San Marino', 'Isle Of Man', 'Jersey', 'Guernsey', 'Gibraltar']
  },
  {
    title: '🌴 Middle East & Gulf Economies',
    locations: ['Kuwait', 'Bahrain', 'Oman', 'Jordan']
  },
  {
    title: '🌎 Americas & High-Value Dollar Markets',
    locations: ['United States', 'Puerto Rico', 'Ecuador', 'El Salvador', 'Panama', 'Bahamas', 'Bermuda', 'Cayman Islands', 'British Virgin Islands', 'Turks And Caicos Islands', 'United States Virgin Islands', 'Caribbean Netherlands']
  },
  {
    title: '🌐 Central & Eastern Europe',
    locations: ['Slovakia', 'Croatia', 'Lithuania', 'Slovenia', 'Latvia', 'Estonia', 'Bulgaria', 'Kosovo', 'Montenegro']
  },
  {
    title: '🌊 Pacific, French Collectivities & Global Hubs',
    locations: ['Guam', 'Northern Mariana Islands', 'American Samoa', 'Marshall Islands', 'Micronesia', 'Palau', 'Timor-Leste', 'Zimbabwe', 'Reunion', 'Guadeloupe', 'Martinique', 'French Guiana', 'Mayotte', 'Saint Martin', 'Saint Barthelemy', 'Saint Pierre And Miquelon', 'Falkland Islands', 'Saint Helena']
  }
];

export const ALL_COUNTRY_LOCATIONS: CountryLocationData[] = [
  {
    "country": "Kuwait",
    "capital": "Kuwait City",
    "currency": "Kuwaiti Dinar (KWD)",
    "currencyCode": "KWD",
    "region": "Middle East",
    "majorCities": [
      "Kuwait City",
      "Al Jahra",
      "Hawalli",
      "As Salimiyah",
      "Sabah as Salim",
      "Al Farwaniyah",
      "Al Ahmadi"
    ],
    "regionalCities": [
      "Al Fahahil",
      "Mahboula",
      "Sabahiya",
      "Mangaf",
      "Ar Riqqah",
      "Abu Halifa",
      "Fintas",
      "Salwa",
      "Ar Rumaythiyah",
      "Jabriya",
      "Bayan",
      "Al Wafrah",
      "Al Khiran"
    ]
  },
  {
    "country": "Bahrain",
    "capital": "Manama",
    "currency": "Bahraini Dinar (BHD)",
    "currencyCode": "BHD",
    "region": "Middle East",
    "majorCities": [
      "Manama",
      "Muharraq",
      "Hamad Town",
      "Riffa"
    ],
    "regionalCities": [
      "Jidhafs",
      "Budaiya",
      "A'ali",
      "Isa Town",
      "Sitra",
      "Al Hidd",
      "Arad",
      "Saar",
      "Sanabis",
      "Galali",
      "Diraz",
      "Malkiya",
      "Zallaq",
      "Awali"
    ]
  },
  {
    "country": "Oman",
    "capital": "Muscat",
    "currency": "Omani Rial (OMR)",
    "currencyCode": "OMR",
    "region": "Middle East",
    "majorCities": [
      "Muscat",
      "Seeb",
      "Salalah",
      "Bawshar",
      "Muttrah",
      "Sohar",
      "Sur",
      "Nizwa"
    ],
    "regionalCities": [
      "As Suwayq",
      "Ibri",
      "Al Buraimi",
      "Saham",
      "Barka",
      "Rustaq",
      "Ibra",
      "Bahla",
      "Khasab",
      "Duqm",
      "Shinas",
      "Samail",
      "Izki",
      "Sinaw",
      "Bidbid",
      "Adam",
      "Haima"
    ]
  },
  {
    "country": "Jordan",
    "capital": "Amman",
    "currency": "Jordanian Dinar (JOD)",
    "currencyCode": "JOD",
    "region": "Middle East",
    "majorCities": [
      "Amman",
      "Zarqa",
      "Irbid",
      "Russeifa",
      "Aqaba"
    ],
    "regionalCities": [
      "Wadi as-Seer",
      "Al Jubaiha",
      "Sahab",
      "Al-Quwaysimah",
      "Madaba",
      "As Salt",
      "Mafraq",
      "Ramtha",
      "Maan",
      "Jerash",
      "Karak",
      "Marj al-Hamam",
      "Tafilah",
      "Fuheis",
      "Wadi Musa",
      "Ajloun",
      "Dhiban",
      "Mahis",
      "Azraq",
      "Safi"
    ]
  },
  {
    "country": "United Kingdom",
    "capital": "London",
    "currency": "Pound Sterling (GBP)",
    "currencyCode": "GBP",
    "region": "Europe",
    "majorCities": [
      "London",
      "Birmingham",
      "Glasgow",
      "Manchester",
      "Leeds",
      "Sheffield",
      "Edinburgh",
      "Liverpool",
      "Bristol",
      "Leicester",
      "Cardiff",
      "Belfast"
    ],
    "regionalCities": [
      "Bradford",
      "Coventry",
      "Nottingham",
      "Newcastle upon Tyne",
      "Wolverhampton",
      "Kingston upon Hull",
      "Plymouth",
      "Stoke-on-Trent",
      "Derby",
      "Southampton",
      "Swansea",
      "Brighton",
      "Milton Keynes",
      "Portsmouth",
      "Aberdeen",
      "Reading",
      "Preston",
      "Dundee",
      "Norwich",
      "Bournemouth",
      "Peterborough",
      "Southend-on-Sea",
      "Sunderland",
      "Middlesbrough",
      "Warrington",
      "Oxford",
      "Newport",
      "Huddersfield",
      "Slough",
      "York",
      "Cambridge",
      "Ipswich",
      "Blackpool",
      "Exeter",
      "Gloucester",
      "Bath",
      "Londonderry",
      "Wrexham",
      "Canterbury",
      "Inverness",
      "Perth",
      "Lisburn",
      "Winchester",
      "Stirling",
      "Newry",
      "Bangor",
      "St Davids"
    ]
  },
  {
    "country": "Switzerland",
    "capital": "Bern",
    "currency": "Swiss Franc (CHF)",
    "currencyCode": "CHF",
    "region": "Europe",
    "majorCities": [
      "Zurich",
      "Geneva",
      "Bern",
      "Basel",
      "Lausanne"
    ],
    "regionalCities": [
      "Winterthur",
      "Lucerne",
      "St. Gallen",
      "Lugano",
      "Biel/Bienne",
      "Fribourg",
      "Zug",
      "Thun",
      "Bellinzona",
      "Chur",
      "Schaffhausen",
      "Sion",
      "Neuchatel",
      "Yverdon-les-Bains",
      "Montreux",
      "Frauenfeld",
      "Aarau",
      "Solothurn",
      "Liestal",
      "Davos"
    ]
  },
  {
    "country": "Germany",
    "capital": "Berlin",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Berlin",
      "Hamburg",
      "Munich",
      "Cologne",
      "Frankfurt",
      "Stuttgart",
      "Dusseldorf",
      "Leipzig",
      "Dortmund",
      "Essen",
      "Bremen",
      "Dresden",
      "Hanover",
      "Nuremberg"
    ],
    "regionalCities": [
      "Duisburg",
      "Bochum",
      "Wuppertal",
      "Bielefeld",
      "Bonn",
      "Munster",
      "Mannheim",
      "Karlsruhe",
      "Augsburg",
      "Wiesbaden",
      "Monchengladbach",
      "Gelsenkirchen",
      "Aachen",
      "Braunschweig",
      "Chemnitz",
      "Kiel",
      "Halle",
      "Magdeburg",
      "Freiburg",
      "Krefeld",
      "Mainz",
      "Lubeck",
      "Erfurt",
      "Rostock",
      "Potsdam",
      "Saarbrucken",
      "Oberhausen",
      "Kassel",
      "Hagen",
      "Hamm",
      "Ludwigshafen",
      "Oldenburg",
      "Osnabruck",
      "Leverkusen",
      "Heidelberg",
      "Darmstadt",
      "Solingen",
      "Regensburg",
      "Ingolstadt",
      "Wurzburg",
      "Ulm",
      "Wolfsburg",
      "Gottingen",
      "Schwerin"
    ]
  },
  {
    "country": "France",
    "capital": "Paris",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Paris",
      "Marseille",
      "Lyon",
      "Toulouse",
      "Nice",
      "Nantes",
      "Montpellier",
      "Strasbourg",
      "Bordeaux",
      "Lille"
    ],
    "regionalCities": [
      "Rennes",
      "Reims",
      "Toulon",
      "Saint-Etienne",
      "Le Havre",
      "Grenoble",
      "Dijon",
      "Angers",
      "Nimes",
      "Villeurbanne",
      "Clermont-Ferrand",
      "Le Mans",
      "Aix-en-Provence",
      "Brest",
      "Tours",
      "Amiens",
      "Limoges",
      "Annecy",
      "Perpignan",
      "Besancon",
      "Metz",
      "Orleans",
      "Rouen",
      "Saint-Denis",
      "Argenteuil",
      "Montreuil",
      "Mulhouse",
      "Caen",
      "Nancy",
      "Roubaix",
      "Tourcoing",
      "Nanterre",
      "Avignon",
      "Poitiers",
      "Versailles",
      "Pau",
      "La Rochelle",
      "Cannes",
      "Ajaccio",
      "Bourges",
      "Chambery",
      "Lorient",
      "Bastia",
      "Chalons-en-Champagne"
    ]
  },
  {
    "country": "Italy",
    "capital": "Rome",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Rome",
      "Milan",
      "Naples",
      "Turin",
      "Palermo",
      "Genoa",
      "Bologna",
      "Florence",
      "Bari",
      "Catania",
      "Venice",
      "Verona",
      "Messina"
    ],
    "regionalCities": [
      "Padua",
      "Trieste",
      "Brescia",
      "Parma",
      "Prato",
      "Taranto",
      "Modena",
      "Reggio Calabria",
      "Reggio Emilia",
      "Perugia",
      "Livorno",
      "Ravenna",
      "Cagliari",
      "Rimini",
      "Foggia",
      "Salerno",
      "Ferrara",
      "Sassari",
      "Latina",
      "Giugliano",
      "Monza",
      "Bergamo",
      "Pescara",
      "Trento",
      "Vicenza",
      "Bolzano",
      "Ancona",
      "Novara",
      "Udine",
      "Andria",
      "Arezzo",
      "Cesena",
      "Lecce",
      "Pesaro",
      "La Spezia",
      "Como",
      "Catanzaro",
      "Potenza",
      "Campobasso",
      "Aosta"
    ]
  },
  {
    "country": "Spain",
    "capital": "Madrid",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Madrid",
      "Barcelona",
      "Valencia",
      "Seville",
      "Zaragoza",
      "Malaga",
      "Murcia",
      "Palma",
      "Las Palmas",
      "Bilbao",
      "Alicante",
      "Cordoba"
    ],
    "regionalCities": [
      "Valladolid",
      "Vigo",
      "Gijon",
      "Hospitalet de Llobregat",
      "Vitoria-Gasteiz",
      "A Coruna",
      "Elche",
      "Granada",
      "Terrassa",
      "Badalona",
      "Oviedo",
      "Cartagena",
      "Jerez de la Frontera",
      "Sabadell",
      "Santa Cruz de Tenerife",
      "Pamplona",
      "Almeria",
      "Donostia-San Sebastian",
      "Burgos",
      "Albacete",
      "Santander",
      "Castellon de la Plana",
      "Ceuta",
      "Melilla",
      "Alcala de Henares",
      "Getafe",
      "Logrono",
      "Badajoz",
      "Huelva",
      "Lleida",
      "Tarragona",
      "Leon",
      "Cadiz",
      "Jaen",
      "Ourense",
      "Santiago de Compostela",
      "Girona",
      "Caceres",
      "Guadalajara",
      "Toledo",
      "Arrecife",
      "Merida",
      "Ibiza",
      "Mahon"
    ]
  },
  {
    "country": "Netherlands",
    "capital": "Amsterdam",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Amsterdam",
      "Rotterdam",
      "The Hague",
      "Utrecht",
      "Eindhoven",
      "Groningen",
      "Tilburg",
      "Almere",
      "Breda",
      "Nijmegen"
    ],
    "regionalCities": [
      "Apeldoorn",
      "Arnhem",
      "Haarlem",
      "Enschede",
      "Haarlemmermeer",
      "Amersfoort",
      "Zaanstad",
      "s-Hertogenbosch",
      "Zwolle",
      "Leiden",
      "Leeuwarden",
      "Maastricht",
      "Dordrecht",
      "Ede",
      "Alphen aan den Rijn",
      "Alkmaar",
      "Emmen",
      "Delft",
      "Venlo",
      "Deventer",
      "Helmond",
      "Sittard",
      "Lelystad",
      "Assen",
      "Middelburg"
    ]
  },
  {
    "country": "Belgium",
    "capital": "Brussels",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Brussels",
      "Antwerp",
      "Ghent",
      "Charleroi",
      "Liege",
      "Bruges",
      "Namur"
    ],
    "regionalCities": [
      "Leuven",
      "Mons",
      "Aalst",
      "Mechelen",
      "La Louviere",
      "Hasselt",
      "Sint-Niklaas",
      "Kortrijk",
      "Ostend",
      "Tournai",
      "Genk",
      "Roeselare",
      "Seraing",
      "Mouscron",
      "Verviers",
      "Turnhout",
      "Wavre",
      "Arlon"
    ]
  },
  {
    "country": "Austria",
    "capital": "Vienna",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Vienna",
      "Graz",
      "Linz",
      "Salzburg",
      "Innsbruck"
    ],
    "regionalCities": [
      "Klagenfurt",
      "Villach",
      "Wels",
      "Sankt Polten",
      "Dornbirn",
      "Wiener Neustadt",
      "Steyr",
      "Feldkirch",
      "Bregenz",
      "Leonding",
      "Klosterneuburg",
      "Baden",
      "Wolfsberg",
      "Krems",
      "Kufstein"
    ]
  },
  {
    "country": "Portugal",
    "capital": "Lisbon",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Lisbon",
      "Porto",
      "Vila Nova de Gaia",
      "Braga",
      "Amadora",
      "Almada",
      "Coimbra",
      "Funchal"
    ],
    "regionalCities": [
      "Matosinhos",
      "Gondomar",
      "Odivelas",
      "Maia",
      "Setubal",
      "Viseu",
      "Agualva-Cacem",
      "Queluz",
      "Aveiro",
      "Barreiro",
      "Ponta Delgada",
      "Leiria",
      "Faro",
      "Guimaraes",
      "Santarem",
      "Portimao",
      "Evora",
      "Castelo Branco",
      "Vila Real",
      "Guarda",
      "Tomar",
      "Viana do Castelo",
      "Braganca",
      "Beja",
      "Angra do Heroismo",
      "Portalegre"
    ]
  },
  {
    "country": "Ireland",
    "capital": "Dublin",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Dublin",
      "Cork",
      "Limerick",
      "Galway",
      "Waterford"
    ],
    "regionalCities": [
      "Drogheda",
      "Dundalk",
      "Swords",
      "Bray",
      "Navan",
      "Kilkenny",
      "Ennis",
      "Carlow",
      "Tralee",
      "Naas",
      "Sligo",
      "Athlone",
      "Mullingar",
      "Wexford",
      "Letterkenny",
      "Clonmel",
      "Greystones",
      "Killarney",
      "Cavan"
    ]
  },
  {
    "country": "Finland",
    "capital": "Helsinki",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Helsinki",
      "Espoo",
      "Tampere",
      "Vantaa",
      "Oulu",
      "Turku",
      "Jyvaskyla",
      "Lahti",
      "Kuopio"
    ],
    "regionalCities": [
      "Pori",
      "Kouvola",
      "Joensuu",
      "Lappeenranta",
      "Hameenlinna",
      "Vaasa",
      "Seinajoki",
      "Rovaniemi",
      "Mikkeli",
      "Kotka",
      "Salo",
      "Porvoo",
      "Kokkola",
      "Hyvinkaa",
      "Jarvenpaa",
      "Nurmijarvi",
      "Rauma",
      "Kajaani",
      "Kerava",
      "Savonlinna",
      "Kemi"
    ]
  },
  {
    "country": "Greece",
    "capital": "Athens",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Athens",
      "Thessaloniki",
      "Patras",
      "Heraklion",
      "Piraeus",
      "Larissa",
      "Volos",
      "Ioannina"
    ],
    "regionalCities": [
      "Peristeri",
      "Kallithea",
      "Acharnes",
      "Kalamaria",
      "Trikala",
      "Chalcis",
      "Alexandroupoli",
      "Xanthi",
      "Katerini",
      "Kalamata",
      "Chania",
      "Kavala",
      "Lamia",
      "Serres",
      "Komotini",
      "Rhodes",
      "Agrinio",
      "Drama",
      "Veria",
      "Kozani",
      "Rethymno",
      "Corfu",
      "Tripoli",
      "Mytilene",
      "Chios",
      "Sparta",
      "Nafplio",
      "Ermoupoli",
      "Agios Nikolaos"
    ]
  },
  {
    "country": "Luxembourg",
    "capital": "Luxembourg City",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Luxembourg City",
      "Esch-sur-Alzette",
      "Differdange",
      "Dudelange"
    ],
    "regionalCities": [
      "Ettelbruck",
      "Diekirch",
      "Mersch",
      "Wiltz",
      "Echternach",
      "Rumelange",
      "Grevenmacher",
      "Remich",
      "Vianden"
    ]
  },
  {
    "country": "Malta",
    "capital": "Valletta",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Birkirkara",
      "Sliema",
      "Valletta",
      "Mosta",
      "Qormi",
      "St. Julians",
      "Victoria"
    ],
    "regionalCities": [
      "Zabbar",
      "Naxxar",
      "San Gwann",
      "Zejtun",
      "Marsaskala",
      "Fgura",
      "Rabat",
      "Mellieha",
      "Zurrieq",
      "Paola",
      "Mdina",
      "Marsa",
      "Xaghra",
      "Nadur",
      "Marsaxlokk"
    ]
  },
  {
    "country": "Cyprus",
    "capital": "Nicosia",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Nicosia",
      "Limassol",
      "Strovolos",
      "Larnaca",
      "Paphos"
    ],
    "regionalCities": [
      "Famagusta",
      "Lakatamia",
      "Kyrenia",
      "Aradippou",
      "Paralimni",
      "Latsia",
      "Ayia Napa"
    ]
  },
  {
    "country": "Monaco",
    "capital": "Monaco",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Monte Carlo",
      "Monaco-Ville"
    ],
    "regionalCities": [
      "La Condamine",
      "Fontvieille",
      "Larvotto",
      "Moneghetti",
      "La Rousse"
    ]
  },
  {
    "country": "Liechtenstein",
    "capital": "Vaduz",
    "currency": "Swiss Franc (CHF)",
    "currencyCode": "CHF",
    "region": "Europe",
    "majorCities": [
      "Schaan",
      "Vaduz"
    ],
    "regionalCities": [
      "Triesen",
      "Balzers",
      "Eschen",
      "Mauren",
      "Triesenberg",
      "Ruggell",
      "Gamprin",
      "Schellenberg",
      "Planken"
    ]
  },
  {
    "country": "Andorra",
    "capital": "Andorra la Vella",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Andorra la Vella",
      "Escaldes-Engordany",
      "Encamp"
    ],
    "regionalCities": [
      "Sant Julia de Loria",
      "La Massana",
      "Ordino",
      "Santa Coloma",
      "Canillo",
      "Pas de la Casa",
      "Arinsal"
    ]
  },
  {
    "country": "San Marino",
    "capital": "City of San Marino",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Serravalle",
      "Dogana",
      "City of San Marino"
    ],
    "regionalCities": [
      "Borgo Maggiore",
      "Domagnano",
      "Fiorentino",
      "Acquaviva",
      "Faetano",
      "Chiesanuova",
      "Montegiardino"
    ]
  },
  {
    "country": "Vatican City",
    "capital": "Vatican City",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Vatican City"
    ],
    "regionalCities": []
  },
  {
    "country": "Isle Of Man",
    "capital": "Douglas",
    "currency": "Manx Pound (IMP)",
    "currencyCode": "IMP",
    "region": "Europe",
    "majorCities": [
      "Douglas",
      "Onchan",
      "Ramsey"
    ],
    "regionalCities": [
      "Peel",
      "Port Erin",
      "Castletown",
      "Port St Mary",
      "Laxey"
    ]
  },
  {
    "country": "Jersey",
    "capital": "Saint Helier",
    "currency": "Jersey Pound (JEP)",
    "currencyCode": "JEP",
    "region": "Europe",
    "majorCities": [
      "Saint Helier",
      "Saint Saviour"
    ],
    "regionalCities": [
      "Saint Brelade",
      "Saint Clement",
      "Grouville",
      "Saint Peter",
      "Saint Lawrence",
      "Saint Martin",
      "Trinity"
    ]
  },
  {
    "country": "Guernsey",
    "capital": "Saint Peter Port",
    "currency": "Guernsey Pound (GGP)",
    "currencyCode": "GGP",
    "region": "Europe",
    "majorCities": [
      "Saint Peter Port",
      "Saint Sampson"
    ],
    "regionalCities": [
      "Vale",
      "Castel",
      "Saint Martin",
      "Saint Anne",
      "Saint Andrew",
      "Saint Peter",
      "Forest",
      "Torteval",
      "Sark"
    ]
  },
  {
    "country": "Gibraltar",
    "capital": "Gibraltar",
    "currency": "Gibraltar Pound (GIP)",
    "currencyCode": "GIP",
    "region": "Europe",
    "majorCities": [
      "Gibraltar"
    ],
    "regionalCities": [
      "Westside",
      "Catalan Bay"
    ]
  },
  {
    "country": "Slovakia",
    "capital": "Bratislava",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Bratislava",
      "Kosice",
      "Presov",
      "Zilina",
      "Nitra",
      "Banska Bystrica",
      "Trnava",
      "Trencin"
    ],
    "regionalCities": [
      "Martin",
      "Poprad",
      "Prievidza",
      "Zvolen",
      "Povazska Bystrica",
      "Michalovce",
      "Nove Zamky",
      "Spisska Nova Ves",
      "Komarno",
      "Levice",
      "Humenne",
      "Bardejov",
      "Liptovsky Mikulas",
      "Piestany",
      "Ruzomberok",
      "Trebisov"
    ]
  },
  {
    "country": "Croatia",
    "capital": "Zagreb",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Zagreb",
      "Split",
      "Rijeka",
      "Osijek",
      "Zadar",
      "Pula",
      "Dubrovnik"
    ],
    "regionalCities": [
      "Slavonski Brod",
      "Karlovac",
      "Velika Gorica",
      "Varazdin",
      "Sibenik",
      "Bjelovar",
      "Kastela",
      "Sisak",
      "Vinkovci",
      "Vukovar",
      "Samobor",
      "Koprivnica",
      "Cakovec",
      "Pozega",
      "Virovitica",
      "Gospic",
      "Krapina"
    ]
  },
  {
    "country": "Lithuania",
    "capital": "Vilnius",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Vilnius",
      "Kaunas",
      "Klaipeda",
      "Siauliai",
      "Panevezys"
    ],
    "regionalCities": [
      "Alytus",
      "Marijampole",
      "Mazeikiai",
      "Jonava",
      "Utena",
      "Kedainiai",
      "Telsiai",
      "Taurage",
      "Ukmerge",
      "Visaginas",
      "Plunge",
      "Kretinga",
      "Palanga",
      "Radviliskis",
      "Druskininkai"
    ]
  },
  {
    "country": "Slovenia",
    "capital": "Ljubljana",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Ljubljana",
      "Maribor",
      "Celje",
      "Kranj",
      "Koper"
    ],
    "regionalCities": [
      "Velenje",
      "Novo Mesto",
      "Ptuj",
      "Jesenice",
      "Nova Gorica",
      "Murska Sobota",
      "Kamnik",
      "Trbovlje",
      "Domzale",
      "Skofja Loka",
      "Izola",
      "Postojna",
      "Slovenj Gradec",
      "Bled",
      "Piran"
    ]
  },
  {
    "country": "Latvia",
    "capital": "Riga",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Riga",
      "Daugavpils",
      "Liepaja",
      "Jelgava",
      "Jurmala"
    ],
    "regionalCities": [
      "Ventspils",
      "Rezekne",
      "Ogre",
      "Valmiera",
      "Jekabpils",
      "Salaspils",
      "Tukums",
      "Cesis",
      "Kuldiga",
      "Olaine",
      "Sigulda",
      "Saldus",
      "Talsi",
      "Dobele",
      "Bauska"
    ]
  },
  {
    "country": "Estonia",
    "capital": "Tallinn",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Tallinn",
      "Tartu",
      "Narva",
      "Parnu"
    ],
    "regionalCities": [
      "Kohtla-Jarve",
      "Viljandi",
      "Rakvere",
      "Maardu",
      "Kuressaare",
      "Sillamae",
      "Valga",
      "Voru",
      "Keila",
      "Johvi",
      "Haapsalu",
      "Paide",
      "Tapa",
      "Rapla"
    ]
  },
  {
    "country": "Bulgaria",
    "capital": "Sofia",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Sofia",
      "Plovdiv",
      "Varna",
      "Burgas",
      "Ruse",
      "Stara Zagora",
      "Pleven",
      "Sliven",
      "Dobrich"
    ],
    "regionalCities": [
      "Shumen",
      "Pernik",
      "Haskovo",
      "Yambol",
      "Pazardzhik",
      "Blagoevgrad",
      "Veliko Tarnovo",
      "Vratsa",
      "Gabrovo",
      "Asenovgrad",
      "Kazanlak",
      "Vidin",
      "Kyustendil",
      "Montana",
      "Targovishte",
      "Lovech",
      "Silistra",
      "Razgrad",
      "Smolyan",
      "Sandanski"
    ]
  },
  {
    "country": "Kosovo",
    "capital": "Pristina",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Pristina",
      "Prizren",
      "Peja",
      "Mitrovica",
      "Ferizaj",
      "Gjakova",
      "Gjilan"
    ],
    "regionalCities": [
      "Podujeva",
      "Vushtrri",
      "Suva Reka",
      "Rahovec",
      "Malisheva",
      "Lipjan",
      "Kamenica",
      "Decan",
      "Istog"
    ]
  },
  {
    "country": "Montenegro",
    "capital": "Podgorica",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Europe",
    "majorCities": [
      "Podgorica",
      "Niksic",
      "Bar",
      "Budva"
    ],
    "regionalCities": [
      "Bijelo Polje",
      "Herceg Novi",
      "Pljevlja",
      "Berane",
      "Ulcinj",
      "Tivat",
      "Cetinje",
      "Kotor",
      "Rozaje",
      "Danilovgrad",
      "Mojkovac"
    ]
  },
  {
    "country": "United States",
    "capital": "Washington, D.C.",
    "currency": "United States Dollar (USD)",
    "currencyCode": "USD",
    "region": "North America",
    "majorCities": [
      "New York",
      "Los Angeles",
      "Chicago",
      "Houston",
      "Phoenix",
      "Philadelphia",
      "San Antonio",
      "San Diego",
      "Dallas",
      "San Jose",
      "San Francisco",
      "Washington",
      "Austin",
      "Jacksonville",
      "Fort Worth",
      "Columbus",
      "Charlotte",
      "Indianapolis",
      "Seattle",
      "Denver",
      "Nashville",
      "Oklahoma City",
      "El Paso",
      "Las Vegas",
      "Boston",
      "Portland",
      "Detroit",
      "Memphis",
      "Louisville",
      "Baltimore",
      "Milwaukee",
      "Albuquerque",
      "Tucson",
      "Fresno",
      "Sacramento",
      "Kansas City",
      "Atlanta",
      "Omaha",
      "Raleigh",
      "Miami",
      "Minneapolis"
    ],
    "regionalCities": [
      "Tulsa",
      "Bakersfield",
      "Tampa",
      "Wichita",
      "Arlington",
      "New Orleans",
      "Aurora",
      "Cleveland",
      "Honolulu",
      "Anaheim",
      "Lexington",
      "Henderson",
      "Corpus Christi",
      "Riverside",
      "Santa Ana",
      "Cincinnati",
      "Orlando",
      "Newark",
      "Pittsburgh",
      "St. Louis",
      "Greensboro",
      "Anchorage",
      "Lincoln",
      "Buffalo",
      "Madison",
      "Boise",
      "Richmond",
      "Baton Rouge",
      "Des Moines",
      "Salt Lake City",
      "Birmingham",
      "Little Rock",
      "Tallahassee",
      "Montgomery",
      "Charleston",
      "Providence",
      "Salem",
      "Jackson",
      "Columbia",
      "Topeka",
      "Hartford",
      "Springfield",
      "Albany",
      "Trenton",
      "Santa Fe",
      "Bismarck",
      "Cheyenne",
      "Carson City",
      "Olympia",
      "Harrisburg",
      "Concord",
      "Jefferson City",
      "Annapolis",
      "Dover",
      "Helena",
      "Frankfort",
      "Augusta",
      "Pierre",
      "Montpelier"
    ]
  },
  {
    "country": "Puerto Rico",
    "capital": "San Juan",
    "currency": "United States Dollar (USD)",
    "currencyCode": "USD",
    "region": "Latin America & Caribbean",
    "majorCities": [
      "San Juan",
      "Bayamon",
      "Carolina",
      "Ponce",
      "Caguas",
      "Guaynabo",
      "Arecibo",
      "Mayaguez"
    ],
    "regionalCities": [
      "Trujillo Alto",
      "Toa Baja",
      "Toa Alta",
      "Vega Baja",
      "Humacao",
      "Aguadilla",
      "Rio Grande",
      "Juana Diaz",
      "Cayey",
      "Guayama",
      "Manati",
      "Yauco",
      "Cataño",
      "Gurabo",
      "Isabela",
      "Cidra",
      "Coamo",
      "San German",
      "Hormigueros",
      "Vieques",
      "Culebra"
    ]
  },
  {
    "country": "Ecuador",
    "capital": "Quito",
    "currency": "United States Dollar (USD)",
    "currencyCode": "USD",
    "region": "Latin America & Caribbean",
    "majorCities": [
      "Guayaquil",
      "Quito",
      "Cuenca",
      "Santo Domingo",
      "Machala",
      "Manta",
      "Duran",
      "Portoviejo",
      "Loja",
      "Ambato"
    ],
    "regionalCities": [
      "Riobamba",
      "Esmeraldas",
      "Quevedo",
      "Milagro",
      "Ibarra",
      "Latacunga",
      "La Libertad",
      "Babahoyo",
      "Tulcan",
      "Santa Elena",
      "Nueva Loja",
      "Salinas",
      "Azogues",
      "Puyo",
      "Tena",
      "Macas",
      "Guaranda",
      "Zamora",
      "Puerto Ayora",
      "Puerto Baquerizo Moreno"
    ]
  },
  {
    "country": "El Salvador",
    "capital": "San Salvador",
    "currency": "United States Dollar (USD)",
    "currencyCode": "USD",
    "region": "Latin America & Caribbean",
    "majorCities": [
      "San Salvador",
      "Santa Ana",
      "Soyapango",
      "San Miguel",
      "Mejicanos",
      "Apopa",
      "Santa Tecla"
    ],
    "regionalCities": [
      "Delgado",
      "Ahuachapan",
      "Ilopango",
      "Usulutan",
      "Zacatecoluca",
      "San Marcos",
      "Sonsonate",
      "Cojutepeque",
      "Metapan",
      "San Vicente",
      "Acajutla",
      "Sensuntepeque",
      "La Union",
      "La Libertad",
      "Chalatenango",
      "San Francisco Gotera"
    ]
  },
  {
    "country": "Panama",
    "capital": "Panama City",
    "currency": "Balboa / US Dollar (PAB/USD)",
    "currencyCode": "USD",
    "region": "Latin America & Caribbean",
    "majorCities": [
      "Panama City",
      "San Miguelito",
      "Colon",
      "La Chorrera",
      "David"
    ],
    "regionalCities": [
      "Arraijan",
      "Tocumen",
      "Santiago",
      "Chitre",
      "Aguadulce",
      "Changuinola",
      "Penonome",
      "Boquete",
      "Puerto Armuelles",
      "La Concepcion",
      "Bocas del Toro",
      "Almirante",
      "Las Tablas",
      "El Porvenir"
    ]
  },
  {
    "country": "Bahamas",
    "capital": "Nassau",
    "currency": "Bahamian Dollar (BSD)",
    "currencyCode": "BSD",
    "region": "Latin America & Caribbean",
    "majorCities": [
      "Nassau",
      "Freeport",
      "Marsh Harbour"
    ],
    "regionalCities": [
      "West End",
      "Andros Town",
      "Dunmore Town",
      "Spanish Wells",
      "George Town",
      "Nicholls Town",
      "Coopers Town",
      "Governors Harbour",
      "Cockburn Town",
      "High Rock",
      "Rock Sound",
      "Alice Town",
      "Arthurs Town",
      "Matthew Town",
      "Clarence Town",
      "Duncan Town"
    ]
  },
  {
    "country": "Bermuda",
    "capital": "Hamilton",
    "currency": "Bermudian Dollar (BMD)",
    "currencyCode": "BMD",
    "region": "Latin America & Caribbean",
    "majorCities": [
      "Hamilton",
      "Pembroke",
      "St. George"
    ],
    "regionalCities": [
      "Warwick",
      "Southampton",
      "Paget",
      "Somerset Village",
      "Flatts Village",
      "Tuckers Town"
    ]
  },
  {
    "country": "Cayman Islands",
    "capital": "George Town",
    "currency": "Cayman Islands Dollar (KYD)",
    "currencyCode": "KYD",
    "region": "Latin America & Caribbean",
    "majorCities": [
      "George Town",
      "West Bay",
      "Bodden Town"
    ],
    "regionalCities": [
      "Savannah",
      "East End",
      "North Side",
      "West End",
      "Blossom Village"
    ]
  },
  {
    "country": "British Virgin Islands",
    "capital": "Road Town",
    "currency": "United States Dollar (USD)",
    "currencyCode": "USD",
    "region": "Latin America & Caribbean",
    "majorCities": [
      "Road Town",
      "Spanish Town"
    ],
    "regionalCities": [
      "East End",
      "Great Harbour",
      "The Settlement"
    ]
  },
  {
    "country": "Turks And Caicos Islands",
    "capital": "Cockburn Town",
    "currency": "United States Dollar (USD)",
    "currencyCode": "USD",
    "region": "Latin America & Caribbean",
    "majorCities": [
      "Providenciales",
      "Cockburn Town"
    ],
    "regionalCities": [
      "Blue Hills",
      "Cockburn Harbour",
      "Balfour Town",
      "Bottle Creek",
      "Kew"
    ]
  },
  {
    "country": "United States Virgin Islands",
    "capital": "Charlotte Amalie",
    "currency": "United States Dollar (USD)",
    "currencyCode": "USD",
    "region": "Latin America & Caribbean",
    "majorCities": [
      "Charlotte Amalie",
      "Christiansted",
      "Frederiksted"
    ],
    "regionalCities": [
      "Charlotte Amalie West",
      "Annas Retreat",
      "Cruz Bay"
    ]
  },
  {
    "country": "Caribbean Netherlands",
    "capital": "Kralendijk",
    "currency": "United States Dollar (USD)",
    "currencyCode": "USD",
    "region": "Latin America & Caribbean",
    "majorCities": [
      "Kralendijk",
      "Oranjestad",
      "The Bottom"
    ],
    "regionalCities": [
      "Rincon",
      "Windwardside",
      "St. Johns",
      "Zions Hill"
    ]
  },
  {
    "country": "Guadeloupe",
    "capital": "Basse-Terre",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Latin America & Caribbean",
    "majorCities": [
      "Les Abymes",
      "Baie-Mahault",
      "Pointe-a-Pitre",
      "Basse-Terre"
    ],
    "regionalCities": [
      "Le Gosier",
      "Petit-Bourg",
      "Sainte-Anne",
      "Le Moule",
      "Sainte-Rose",
      "Capesterre-Belle-Eau",
      "Morne-a-l'Eau",
      "Le Lamentin"
    ]
  },
  {
    "country": "Martinique",
    "capital": "Fort-de-France",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Latin America & Caribbean",
    "majorCities": [
      "Fort-de-France",
      "Le Lamentin",
      "Le Robert"
    ],
    "regionalCities": [
      "Schoelcher",
      "Sainte-Marie",
      "Le Francois",
      "Ducos",
      "Riviere-Pilote",
      "Trinite",
      "Le Marin",
      "Saint-Joseph",
      "Sainte-Luce"
    ]
  },
  {
    "country": "French Guiana",
    "capital": "Cayenne",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Latin America & Caribbean",
    "majorCities": [
      "Cayenne",
      "Saint-Laurent-du-Maroni",
      "Kourou"
    ],
    "regionalCities": [
      "Matoury",
      "Remire-Montjoly",
      "Macouria",
      "Maripasoula",
      "Mana",
      "Apatou",
      "Grand-Santi"
    ]
  },
  {
    "country": "Saint Martin",
    "capital": "Marigot",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Latin America & Caribbean",
    "majorCities": [
      "Marigot",
      "Grand-Case"
    ],
    "regionalCities": [
      "Quartier d'Orleans",
      "Sandy Ground",
      "Cul-de-Sac",
      "Colombier"
    ]
  },
  {
    "country": "Saint Barthelemy",
    "capital": "Gustavia",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Latin America & Caribbean",
    "majorCities": [
      "Gustavia",
      "Saint-Jean"
    ],
    "regionalCities": [
      "Lorient",
      "Corossol",
      "Flamands"
    ]
  },
  {
    "country": "Saint Pierre And Miquelon",
    "capital": "Saint-Pierre",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "North America",
    "majorCities": [
      "Saint-Pierre"
    ],
    "regionalCities": [
      "Miquelon-Langlade"
    ]
  },
  {
    "country": "Falkland Islands",
    "capital": "Stanley",
    "currency": "Falkland Islands Pound (FKP)",
    "currencyCode": "FKP",
    "region": "Latin America & Caribbean",
    "majorCities": [
      "Stanley"
    ],
    "regionalCities": [
      "Mount Pleasant",
      "Goose Green",
      "Port Howard"
    ]
  },
  {
    "country": "Guam",
    "capital": "Hagatna",
    "currency": "United States Dollar (USD)",
    "currencyCode": "USD",
    "region": "Pacific & Africa",
    "majorCities": [
      "Dededo",
      "Tamuning",
      "Hagåtña",
      "Yigo"
    ],
    "regionalCities": [
      "Mangilao",
      "Barrigada",
      "Chalan Pago-Ordot",
      "Santa Rita",
      "Yona",
      "Agat",
      "Sinajana",
      "Hagåt",
      "Agana Heights",
      "Talofofo",
      "Inarajan",
      "Asan-Maina",
      "Merizo",
      "Piti",
      "Umatac"
    ]
  },
  {
    "country": "Northern Mariana Islands",
    "capital": "Saipan",
    "currency": "United States Dollar (USD)",
    "currencyCode": "USD",
    "region": "Pacific & Africa",
    "majorCities": [
      "Saipan",
      "Garapan",
      "Capitol Hill"
    ],
    "regionalCities": [
      "Chalan Kanoa",
      "San Jose",
      "Songsong"
    ]
  },
  {
    "country": "American Samoa",
    "capital": "Pago Pago",
    "currency": "United States Dollar (USD)",
    "currencyCode": "USD",
    "region": "Pacific & Africa",
    "majorCities": [
      "Tafuna",
      "Pago Pago",
      "Nuuuli"
    ],
    "regionalCities": [
      "Fagatogo",
      "Faleniu",
      "Leone",
      "Aua",
      "Utulei",
      "Tau",
      "Fitiuta"
    ]
  },
  {
    "country": "Marshall Islands",
    "capital": "Majuro",
    "currency": "United States Dollar (USD)",
    "currencyCode": "USD",
    "region": "Pacific & Africa",
    "majorCities": [
      "Majuro",
      "Ebeye"
    ],
    "regionalCities": [
      "Laura",
      "Rita",
      "Jabor",
      "Wotje"
    ]
  },
  {
    "country": "Micronesia",
    "capital": "Palikir",
    "currency": "United States Dollar (USD)",
    "currencyCode": "USD",
    "region": "Pacific & Africa",
    "majorCities": [
      "Weno",
      "Palikir"
    ],
    "regionalCities": [
      "Kolonia",
      "Colonia",
      "Nett",
      "Tofol"
    ]
  },
  {
    "country": "Palau",
    "capital": "Ngerulmud",
    "currency": "United States Dollar (USD)",
    "currencyCode": "USD",
    "region": "Pacific & Africa",
    "majorCities": [
      "Koror",
      "Ngerulmud"
    ],
    "regionalCities": [
      "Airai",
      "Meyuns",
      "Melekeok",
      "Kloulklubed"
    ]
  },
  {
    "country": "Timor-Leste",
    "capital": "Dili",
    "currency": "United States Dollar (USD)",
    "currencyCode": "USD",
    "region": "Pacific & Africa",
    "majorCities": [
      "Dili",
      "Baucau"
    ],
    "regionalCities": [
      "Maliana",
      "Lospalos",
      "Liquica",
      "Ainaro",
      "Viqueque",
      "Aileu",
      "Same",
      "Suai",
      "Manatuto",
      "Pante Macassar",
      "Gleno"
    ]
  },
  {
    "country": "Zimbabwe",
    "capital": "Harare",
    "currency": "United States Dollar (USD)",
    "currencyCode": "USD",
    "region": "Pacific & Africa",
    "majorCities": [
      "Harare",
      "Bulawayo",
      "Chitungwiza",
      "Mutare",
      "Gweru"
    ],
    "regionalCities": [
      "Kwekwe",
      "Masvingo",
      "Kadoma",
      "Chinhoyi",
      "Norton",
      "Marondera",
      "Chegutu",
      "Bindura",
      "Zvishavane",
      "Beitbridge",
      "Hwange",
      "Victoria Falls",
      "Redcliff",
      "Rusape",
      "Chipinge",
      "Karoi",
      "Kariba",
      "Gwanda",
      "Shurugwi"
    ]
  },
  {
    "country": "Reunion",
    "capital": "Saint-Denis",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Pacific & Africa",
    "majorCities": [
      "Saint-Denis",
      "Saint-Paul",
      "Saint-Pierre",
      "Le Tampon"
    ],
    "regionalCities": [
      "Saint-Andre",
      "Saint-Louis",
      "Saint-Benoit",
      "Saint-Joseph",
      "Le Port",
      "Saint-Leu",
      "Sainte-Marie",
      "La Possession",
      "Sainte-Suzanne",
      "Bras-Panon"
    ]
  },
  {
    "country": "Mayotte",
    "capital": "Mamoudzou",
    "currency": "Euro (EUR)",
    "currencyCode": "EUR",
    "region": "Pacific & Africa",
    "majorCities": [
      "Mamoudzou",
      "Koungou",
      "Dzaoudzi"
    ],
    "regionalCities": [
      "Dembeni",
      "Bandraboua",
      "Tsingoni",
      "Pamandzi",
      "Sada",
      "Bandrele",
      "Chirongui"
    ]
  },
  {
    "country": "Saint Helena",
    "capital": "Jamestown",
    "currency": "Saint Helena Pound (SHP)",
    "currencyCode": "SHP",
    "region": "Pacific & Africa",
    "majorCities": [
      "Jamestown"
    ],
    "regionalCities": [
      "Half Tree Hollow",
      "Georgetown",
      "Edinburgh of the Seven Seas",
      "Longwood"
    ]
  }
];
